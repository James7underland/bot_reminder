...
- **7.2:** планировщик — напоминания из `reminder_at`, отдельные
  уведомления о просрочке из `deadline` (анти-дубль у каждого).
- **7.3:** команды/хелп под новую модель (slash как фолбэк, минимально).

### Фаза 8 — HTTP API + Telegram Mini App
- FastAPI: раздача Mini App + REST поверх функций БД; авторизация по
  Telegram `initData` (чистая функция, под тесты).
- Cloudflare Tunnel → стабильный HTTPS (нюанс стабильного URL решается
  здесь: именованный туннель / бесплатный домен на Cloudflare).
- Фронтенд: список задач, чекбоксы, важность, календарь срока и
  напоминания, подсветка просроченных (как в MS To Do).
- Тестовая дисциплина: вся логика — в Python под pytest (≥90%); Mini App
  — тонкий клиент над протестированным API.
